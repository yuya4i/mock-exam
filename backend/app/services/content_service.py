...
def _is_http_url(value) -> bool:
    """Return True iff *value* is a non-empty string with an http(s) scheme.

    The frontend renders ``pages[].url`` and ``source_info.source`` into
    ``<a :href>`` attributes; any other scheme (``javascript:``,
    ``data:``, ``vbscript:``, ``file:``...) is potentially XSS-able when
    the user clicks. We gate at the backend boundary so the frontend
    can trust what arrives over the wire (defense-in-depth lives in the
    FE too).
    """
...
class CamoufoxPlugin(SourcePlugin):
    ...
    def _build_result(
        ...
        # SEC-12 / FRONTEND-9: drop any page whose url is not an
        # explicit http(s) link. The frontend renders these as
        # <a :href="p.url"> and would happily activate javascript: /
        # data: / vbscript: schemes from a malicious scrape target.
        # The backend is the only place that has a single chokepoint
        # for this — do the gating here once.
        safe_pages = [p for p in pages if _is_http_url(p.get("url"))]

        title = safe_pages[0]["title"] if safe_pages else source
